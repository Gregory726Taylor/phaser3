...
const { CanvasRenderer } = require('phaser');
const CounterClockwise = require('../../../../src/math/angle/CounterClockwise');
const RadToDeg = require('../../../../src/math/RadToDeg');
const Wrap = require('../../../../src/math/Wrap');
 ...
 * @private
 *
 * @param {CanvasRenderer} renderer - A reference to the current active Canvas renderer.
 * @param {SpineGameObject} src - The Game Object being rendered in this call.
 * @param {Camera} camera - The Camera that is rendering the Game Object.
 * @param {TransformMatrix} parentMatrix - This transform matrix is defined if the game object is nested
 */
const SpineGameObjectCanvasRenderer = function (renderer, src, camera, parentMatrix) {
  const context = renderer.currentContext;

  const plugin = src.plugin;
  const skeleton = src.skeleton;
  const skeletonRenderer = plugin.skeletonRenderer;

  const camMatrix = renderer._tempMatrix1;
  const spriteMatrix = renderer._tempMatrix2;
  const calcMatrix = renderer._tempMatrix3;
